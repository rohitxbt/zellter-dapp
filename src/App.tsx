...
function InnerApp() {
    ...
    const createVault = async () => {
        ...
        try {
            ...
            try {
                ...
                throw new Error(sendError.reason || sendError.message || "Failed to send transaction");
            }
    ...
    const pingVault = async (vaultId: number) => {
        ...
        try {
            ...
            console.log("Sending ping for vault:", vaultId);

            let tx;
            try {
                tx = await contract.ping(BigInt(vaultId), { gasLimit: 100000n });
                console.log("Ping transaction sent:", tx.hash);
            } catch (sendError: any) {
                console.error("Ping send failed:", sendError);
                throw new Error(sendError.reason || sendError.message || "Failed to send ping");
            }
            ...
            try {
                const receipt = await tx.wait();
                console.log("Ping confirmed:", receipt);
            } catch (waitError: any) {
                console.error("Ping transaction failed:", waitError);
                throw new Error("Ping transaction reverted");
            }
            ...
            showToast("Error", e.message || "Ping failed.", "error");
        } finally {
                ...
                {view === 'hero' && (
                    <section className="w-full text-center max-w-4xl mx-auto mb-16 relative perspective-1000 animate-float-slow">
                        ...
                        <div className="flex justify-center gap-4">
                            <button onClick={() => { handleConnect(); }} className="btn-core btn-glow bg-black text-white px-8 py-4 rounded-full font-bold text-sm shadow-xl hover:shadow-2xl hover:scale-105 flex items-center gap-3 transition-all shimmer-trigger">
                                    ...
                                    <button onClick={createVault} disabled={isLoading} className="w-full group relative overflow-hidden bg-black text-white rounded-[1.5rem] p-6 text-left shadow-2xl transition-all hover:scale-[1.02] btn-slide-fill hover:shadow-yellow">
                                        <div className="relative z-10 flex justify-between items-center">
                                            ...
                                            <div className="w-12 h-12 rounded-full border border-gray-600 flex items-center justify-center group-hover:bg-zyellow group-hover:text-black transition-all">
                                                <span className="material-symbols-outlined icon-bounce">lock</span>
                                            </div>
                                        </div>
                                    </button>
                                </div>
                                        ...
                                        {myVaults.map((vault) => (
                                            <div key={vault.id} className="vault-card card-shine bg-white p-6 rounded-3xl border border-gray-100 shadow-soft flex flex-col gap-6">
                                                <div className="flex justify-between items-start">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <span className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center" title={vault.payloadType === PayloadType.TEXT ? "Text" : "File"}>
                                                            <span className="material-symbols-outlined text-sm text-gray-600">{vault.payloadType === PayloadType.TEXT ? "short_text" : "attach_file"}</span>
                                                        </span>
                                                        <span className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center" title={vault.storageMode === StorageMode.ONCHAIN_FHE ? "On-Chain FHE" : "Off-Chain IPFS"}>
                                                            <span className="material-symbols-outlined text-sm text-gray-600">{vault.storageMode === StorageMode.ONCHAIN_FHE ? "enhanced_encryption" : "cloud"}</span>
                                                        </span>
                                                    </div>
                                                </div>
                                                <div className={`px-3 py-1 text-white text-xs font-bold rounded-full w-fit ${getStatus(vault).color} transition-colors duration-500 ${getStatus(vault).active ? 'vault-status-active' : ''}`}>
                                                    {getStatus(vault).text}
                                                </div>
                                                <div className="space-y-2">
                                                    <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden relative">
                                                        <div
                                                            className="h-full bg-black transition-all duration-1000 ease-linear relative overflow-hidden progress-glow"
                                                            style={{ width: `${getProgressWidth(vault.lastPing, vault.heartbeat)}%` }}
                                                        >
                                                            <div className="absolute inset-0 bg-white/20 progress-striped"></div>
                                                        </div>
                                                    </div>
                                                ...
                                                <button
                                                    ...
                                                    className={`w-full py-4 font-bold rounded-xl flex items-center justify-center gap-2 transition-all btn-magnetic ${pingLoading[vault.id] ? 'bg-gray-200 text-gray-400' : 'bg-zyellow hover:bg-black hover:text-white text-black hover:shadow-black'}`}
                                                >
                                        ...
                                        <div key={i} className="vault-card card-shine bg-white p-6 rounded-3xl border border-gray-100 shadow-soft">
                                            <div className="mb-4">
                                                <div className="flex justify-between">
                                                    ...
                                                    <div className="flex gap-1">
                                                        ...
                                                        <span className="w-6 h-6 rounded bg-gray-100 flex items-center justify-center">
                                                            <span className="material-symbols-outlined text-xs text-gray-600">{v.storageMode === StorageMode.ONCHAIN_FHE ? "enhanced_encryption" : "cloud"}</span>
                                                        </span>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex gap-2">
                                                {!v.claimed && getStatus(v).active === false ? (
                                                    <button onClick={() => claimVault(v.owner, v.id)} className="flex-1 py-3 bg-black text-white font-bold rounded-xl hover:bg-zyellow hover:text-black transition-all btn-magnetic hover:shadow-yellow">CLAIM</button>
                                                ) : v.claimed ? (
                                                    <button onClick={() => viewSecret(v.owner, v.id, v.storageMode, v.payloadType)} className="flex-1 py-3 bg-gray-900 text-white font-bold rounded-xl hover:bg-gray-800 transition-all btn-magnetic flex items-center justify-center gap-2">
                                                        <span className="material-symbols-outlined text-sm icon-pulse">visibility</span> VIEW SECRET
                                                    </button>
                                                ) : (
